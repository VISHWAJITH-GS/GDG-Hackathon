...
export default function TeamLeaderboard() {
  ...
  // Compute team scores live from source data (reports + reviews)
  // This avoids dependency on the `teams` aggregation collection.
  useEffect(() => {
    let reportsData = []
    let reviewsData = []
    ...
    function rebuild() {
      if (!reportsLoaded || !reviewsLoaded) return

      const teamMap = new Map()

      // Base points: +20 per cleared complaint; count dispatched & cleared
      ...
      // Rating bonus: +rating×5 per review; accumulate for average
      reviewsData.forEach(rv => {
        const name = rv.team_name
        if (!name) return
        if (!teamMap.has(name)) teamMap.set(name, mkTeam(name))
        const t     = teamMap.get(name)
        const bonus = Math.round((rv.rating ?? 0) * 5)
        t.total_points += bonus
        t.rating_sum   += (rv.rating ?? 0)
        t.rating_count += 1
      })

      // Derive average_rating
      teamMap.forEach(t => {
        if (t.rating_count > 0) {
          t.average_rating = Math.round((t.rating_sum / t.rating_count) * 10) / 10
        }
      })
    ...
    // Listener 1: dispatched + cleared reports
    const unsubReports = onSnapshot(
      query(collection(db, 'reports'), where('status', 'in', ['dispatched', 'cleared'])),
      snap => { reportsData = snap.docs.map(d => ({ id: d.id, ...d.data() })); reportsLoaded = true; rebuild() },
      err  => { console.error('[Leaderboard] reports:', err); reportsLoaded = true; rebuild() },
    )

    // Listener 2: all reviews
    const unsubReviews = onSnapshot(
      collection(db, 'reviews'),
      snap => { reviewsData = snap.docs.map(d => ({ id: d.id, ...d.data() })); reviewsLoaded = true; rebuild() },
      err  => { console.error('[Leaderboard] reviews:', err); reviewsLoaded = true; rebuild() },
    )

    return () => { unsubReports(); unsubReviews() }
  }, [])
